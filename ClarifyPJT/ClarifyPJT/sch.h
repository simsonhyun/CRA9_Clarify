...
public:
	...
	vector<Employee> searchWithPrint(InputParameter input) {
		return searchPolicy[input.column]->searchByPolicyWithPrint(input, *getDataManager());
	}
	int searchWithoutPrint(InputParameter input) {
		return searchPolicy[input.column]->searchByPolicyWithoutPrint(input, *getDataManager());
	}
